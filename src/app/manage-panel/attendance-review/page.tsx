...
export default function AttendanceReviewPage() {
  ...
  async function submitCorrection() {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) { showToast(d.error || 'Hata', false); return }
      showToast('Yoklama düzeltildi, bildirim iptal edildi')
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {notifications.map((n, i) => {
                  ...
                  const guardians = n.student.guardians
                  const hasEmail  = guardians.some(g => g.receivesEmail && g.email)
                  const hasPhone  = guardians.some(g => g.receivesSMS && g.phone)
                  const noGuardian = guardians.length === 0
                  ...
                  return (
                    <tr key={n.id} className={`border-b border-gray-50 hover:bg-gray-50/50 ${i % 2 === 0 ? '' : 'bg-gray-50/30'}`}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        {n.attendance.notes && (
                          <div className="text-xs text-gray-400 mt-0.5">{n.attendance.notes}</div>
                        )}
                        {n.status === 'CORRECTED' && n.correctedTo && (
                          <div className="text-xs text-purple-600 mt-0.5">
                            → {ATTEND_MAP[n.correctedTo]?.label ?? n.correctedTo}
                          </div>
                        )}
                      </td>
                          ...
                          <div className="space-y-0.5">
                            {guardians.slice(0, 2).map((g, gi) => (
                              <div key={gi} className="text-xs text-gray-600">
                                ...
                                {!g.receivesEmail && !g.receivesSMS && <span className="ml-1 text-gray-400">–</span>}
                              </div>
                            ))}
                            {guardians.length > 2 && <div className="text-xs text-gray-400">+{guardians.length - 2} daha</div>}
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {n.status === 'PENDING' && (
                            <>
                              ...
                              <button
                                onClick={() => { setCorrectModal(n); setCorrectTo('PRESENT'); setReviewNote('') }}
                          ...
                          {n.status === 'FAILED' && (
                            <button
                              ...
                              disabled={actionLoading === n.id + '_retry'}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 pb-6 flex gap-3 justify-end">
              ...
              <button
                ...
                disabled={actionLoading === 'correct'}
