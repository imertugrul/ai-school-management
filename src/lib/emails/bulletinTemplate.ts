...
export function buildBulletinEmail(data: BulletinEmailData): { subject: string; html: string } {
  ...
  const subject = `${monthStr} Aylık Performans Bülteni — ${data.studentName}`
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.08);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1e40af 0%,#7c3aed 100%);padding:32px 40px;text-align:center;">
            <p style="margin:0 0 8px;font-size:13px;color:rgba(255,255,255,.7);letter-spacing:1px;text-transform:uppercase;">${data.schoolName ?? 'Okul'}</p>
            <h1 style="margin:0;font-size:26px;font-weight:700;color:#ffffff;">${monthStr} Performans Bülteni</h1>
        ...
        <tr><td style="padding:32px 40px;">
          ...
          <p style="margin:0 0 24px;font-size:15px;color:#374151;">
            Sayın <strong>${data.guardianName}</strong>,<br/>
            <strong>${data.teacherName}</strong> tarafından hazırlanan ${monthStr} ayı aylık performans bültenini aşağıda bulabilirsiniz.
          </p>

          <!-- Devamsızlık -->
          <h2 style="margin:0 0 12px;font-size:14px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;">Devam Durumu</h2>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;margin-bottom:28px;overflow:hidden;">
            <tr>
              <td style="padding:16px;text-align:center;border-right:1px solid #e5e7eb;">
                ...
                <div style="font-size:12px;color:#6b7280;margin-top:2px;">Devam</div>
              </td>
              <td style="padding:16px;text-align:center;border-right:1px solid #e5e7eb;">
                <div style="font-size:28px;font-weight:700;color:#dc2626;">${data.attendanceAbsent}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px;">Devamsız</div>
              </td>
              <td style="padding:16px;text-align:center;border-right:1px solid #e5e7eb;">
                <div style="font-size:28px;font-weight:700;color:#d97706;">${data.attendanceLate}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px;">Geç</div>
              </td>
              <td style="padding:16px;text-align:center;">
                <div style="font-size:28px;font-weight:700;color:#1e40af;">${attendancePct}%</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px;">Devam Oranı</div>
              </td>
            </tr>
          </table>
          ...
          <!-- Notlar -->
          <h2 style="margin:0 0 12px;font-size:14px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;">Not Ortalaması</h2>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;margin-top:8px;">
            <thead>
              <tr style="background:#f1f5f9;">
                <th style="padding:8px 12px;text-align:left;font-size:12px;color:#6b7280;font-weight:600;">Ders</th>
                <th style="padding:8px 12px;text-align:right;font-size:12px;color:#6b7280;font-weight:600;">Ortalama</th>
              </tr>
            </thead>
          ...
          <!-- Değerlendirme -->
          <h2 style="margin:0 0 12px;font-size:14px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;">Öğretmen Değerlendirmesi</h2>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;margin-bottom:28px;">
            <tr><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">
              <span style="font-size:13px;color:#374151;font-weight:600;">Katılım: </span>
              <span style="font-size:13px;color:#6b7280;">${ratingLabel(data.participationRating)}</span>
            </td></tr>
            <tr><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">
              <span style="font-size:13px;color:#374151;font-weight:600;">Davranış: </span>
              <span style="font-size:13px;color:#6b7280;">${ratingLabel(data.behaviorRating)}</span>
            </td></tr>
            <tr><td style="padding:12px 16px;">
              <span style="font-size:13px;color:#374151;font-weight:600;">Ev Ödevi: </span>
              <span style="font-size:13px;color:#6b7280;">${ratingLabel(data.homeworkRating)}</span>
            </td></tr>
          </table>

          ${data.strengthAreas ? `
          <h2 style="margin:0 0 8px;font-size:14px;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:.5px;">Güçlü Yönler</h2>
          ...
          <h2 style="margin:0 0 8px;font-size:14px;font-weight:700;color:#d97706;text-transform:uppercase;letter-spacing:.5px;">Gelişim Alanları</h2>
          ...
          ${data.teacherComment ? `
          <h2 style="margin:0 0 8px;font-size:14px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-spacing:.5px;">Öğretmen Yorumu</h2>
          ...
          <p style="margin:32px 0 0;font-size:13px;color:#9ca3af;text-align:center;">
            Bu bülten ${data.teacherName} tarafından oluşturulmuştur.<br/>
